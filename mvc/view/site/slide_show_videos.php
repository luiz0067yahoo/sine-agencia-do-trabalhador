<?php
	...
	if (isset($result_slide_show["data"])){$album_videos_slide_show=$result_slide_show["data"];}
...
<div id="carouselExampleIndicators" class="carousel slide block-row-2" data-ride="carousel">
  ...
  <div class="carousel-inner">
    <?php for($i=0;$i<count($album_videos_slide_show);$i++){ 
        $video=$album_videos_slide_show[$i][array_search('video',$album_videos_campos_slide_show)];//video
        $nome=$album_videos_slide_show[$i][array_search('nome',$album_videos_campos_slide_show)];//nome
        $url=$album_videos_slide_show[$i][array_search('url',$album_videos_campos_slide_show)];//url
        for ($menu_index_slide_show = 1; $menu_index_slide_show <= count($menus); $menu_index_slide_show++){
            if($menus[$menu_index_slide_show-1][1]==$album_videos_slide_show[$i][array_search('menu_principal',$album_videos_campos_slide_show)])
                break;
        }
    ?>
    ...
    <p class="w-100 text-center mt-1" ><a href="<?php echo $base_url."/".$url;?>" class="link-color-<?php echo $menu_index_slide_show; ?>">  <?php echo $nome;?>  </a></p>
    </div>
    <?php } ?>
  </div>
