	...
	if (!function_exists("issetEmpty")){
		function emptyParameter($parameter)
		{
			...
			return isset($_POST[$parameter])?empty($_POST[$parameter]):(
			    isset($_GET[$parameter])?empty($_GET[$parameter]):(
			        isset($_PUT[$parameter])?empty($_PUT[$parameter]):(
			            isset($_DELETE[$parameter])?empty($_DELETE[$parameter]):(
			                isset($_PATCH[$parameter])?empty($_PATCH[$parameter]):false
		                )
		            )
			    )
			);
